...
from insurance_base_agent import InsuranceBaseAgent
...
class PromptBasedAssetResearcher(InsuranceBaseAgent):
    """
    ...
    def __init__(self, 
        ...
        """
        # Initialize parent class with AI backend configuration
        super().__init__(model_type, model_name, api_key, device)
        ...
        logger.info(f"PromptBasedAssetResearcher initialized (Backend: {backend_info['backend']}, Device: {self.device})")
    ...
    def _correct_vehicle_make(self, make: str) -> str:
        ...
        if self.use_ai:
            try:
                ...
                corrected_make = self._generate_with_ai(prompt, max_tokens=50).strip().upper()
